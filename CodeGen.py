...
def fill_active_info():
    i = exp_list.top
    while i in range(exp_list.base, exp_list.top+1):
        ls = exp_list.get(i)        # 每条中间代码（以list变量存储）
        j = 0
        for j in range(1, len(ls)):               # 如果参数不存在
...
def gen_transfer_call(index):
    start = index
    for index in range(start, exp_list.top + 1):
        ...
        if ls[0] == 'Call':                  # 处理当前参数调用
            ...
            for i in range(0, call_stack.top+1):  # 把实参的值传给形参单元
                if tl <= tb.parm:
...
# 生成目标代码,正向生成
def gen_aim_code():
    for i in range(exp_list.base, exp_list.top + 1):
        ls = exp_list.get(i)  # 每条中间代码（以list变量存储）
        if ls[0] == '+':
            gen_add_sub(ls, 'ADD')
        elif ls[0] == '-':
